...
namespace PeopleAPI.Tests.Domain.User; 
...
public class UserTest
{
    ...
    public void CreateUser_WithValidData_ShouldSucceed()
    {
        ...
        var user = PeopleAPI.Domain.Entities.User.Create(email, password);
    ...
    public void CreateUser_WithInvalidEmail_ShouldThrowException(string email)
    {
        var ex = Assert.Throws<DomainException>(() =>
            PeopleAPI.Domain.Entities.User.Create(email, "Senha123"));
    ...
    {
        var ex = Assert.Throws<DomainException>(() =>
            PeopleAPI.Domain.Entities.User.Create("usuario@email.com", password));
    ...
    {
        var result = PeopleAPI.Domain.Entities.User.ValidateEmail(email);
    ...
    {
        var result = PeopleAPI.Domain.Entities.User.ValidatePassword(password);
        Assert.Equal(expected, result);
    }
}
